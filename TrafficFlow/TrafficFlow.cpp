...
void runParallel(std::vector<int>& local_road, int rank, int size) {
    ...
    for (int t = 0; t < ITERATIONS; ++t) {
        ...
        // Cars look forward (right). We need the left edge of our right neighbor.
        // Therefore, we send our left edge to our left neighbor.
        for (int i = 0; i < MAX_SPEED; i++) {
            left_boundary_send[i] = local_road[i];
        }

        MPI_Request reqs[2];
        // Non-blocking send left, receive right
        ...
        // COMPUTE INNER CELLS (Overlap computation with communication)
#pragma omp parallel 
        {
            std::mt19937 local_rng(rank * 1000 + omp_get_thread_num() + t);
            std::uniform_real_distribution<double> dist(0.0, 1.0);

            // Compute up to local_length - MAX_SPEED to avoid needing ghost cells yet
#pragma omp for
        ...
        // COMPUTE BOUNDARY CELLS (Now that ghost cells are here)
#pragma omp parallel 
        {
...
#pragma omp for
        ...
        // Visualization Export (Rank 0 only)
        if (rank == 0) {
...
int main(int argc, char** argv) {
    ...
    double start_time, end_time, seq_time = 0.0, par_time = 0.0;
    ...
    start_time = MPI_Wtime();
    runParallel(local_road, rank, size);
    MPI_Barrier(MPI_COMM_WORLD);
